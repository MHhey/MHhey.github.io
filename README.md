...
- Hero animation: a supply chain that cycles between two states — ordering swings amplifying toward
  the supplier end, then settling into a level, predictable flow. The settling half is the point: it
  reads as a problem being solved, which is what your work does. 14-second loop, computed live.
  Under `prefers-reduced-motion` it renders the settled state, pauses when the tab is hidden, and is
  hidden in print. Only two labels ("On site", "Suppliers") so it stays legible to a non-technical
  reader; the plain-English caption underneath does the rest.
